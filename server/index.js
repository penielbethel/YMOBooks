...
app.post('/api/update-company', async (req, res) => {
  try {
    ...
    app.post('/api/invoice/create', async (req, res) => {
      try {
        let { companyId, invoiceNumber, invoiceDate, dueDate, customer = {}, items = [], template, brandColor, companyOverride } = req.body;
        companyId = (companyId || '').trim();
        ...
        app.post('/api/receipt/create', async (req, res) => {
          try {
            let { companyId, invoiceNumber, receiptNumber, receiptDate, customer = {}, amountPaid, items = [] } = req.body;
            companyId = (companyId || '').trim();
            if (!companyId) return res.status(400).json({ success: false, message: 'companyId is required' });
            // ...
            // ...
            ...
            app.get('/api/receipts', async (req, res) => {
              try {
                let { companyId, months = 6 } = req.query;
                companyId = (companyId || '').trim();
                if (!companyId) return res.status(400).json({ success: false, message: 'companyId is required' });
                // ...
                // ...
                            ...
                            app.get('/api/finance/expenses-daily', async (req, res) => {
                              try {
                                ...
                                app.post('/api/finance/expenses-daily', async (req, res) => {
                                  try {
                                    let { companyId, month, day, amount } = req.body || {};
                                    companyId = (companyId || '').trim();
                                    if (!companyId) return res.status(400).json({ success: false, message: 'Missing companyId' });
                                ...
                                function drawReceiptByTemplate(doc, company, rctNo, receiptDate, invoiceNumber, customer, amountPaid, items = []) {
                                  ...
                                  const theme = {
                                    classic: { primary: '#10b981', tableHeader: '#f8fafc' },
                                    modern: { primary: '#10b981', tableHeader: '#f0fdf4' },
                                    minimal: { primary: '#0f172a', tableHeader: '#f1f5f9' },
                                    bold: { primary: '#10b981', tableHeader: '#f1f5f9' },
                                    compact: { primary: '#10b981', tableHeader: '#f8fafc' },
                                  }[template] || { primary: '#10b981', tableHeader: '#f8fafc' };
                                  ...
                                  // Header - Confirmed Green or Brand Primary
                                  ...
                                  doc.rect(pageLeft, y, pageWidth, 25).fill(theme.tableHeader);
                                  doc.fillColor(theme.primary).fontSize(10).text('SERVICES/ITEMS PAID FOR', pageLeft + 10, y + 8);
                                  doc.text('QTY', pageLeft + colDesc, y + 8, { width: colQty, align: 'center' });
                                  doc.text('PRICE', pageLeft + colDesc + colQty, y + 8, { width: colPrice, align: 'right' });
                                  doc.text('TOTAL', pageLeft + colDesc + colQty + colPrice, y + 8, { width: colTotal - 10, align: 'right' });
                                  ...
                                  activeItems.forEach((it, i) => {
                                    ...
                                    doc.fillColor('#334155').fontSize(10).text(it.description || it.desc || '', pageLeft + 10, y + 6, { width: colDesc });
                                    doc.text(String(it.qty || 0), pageLeft + colDesc, y + 6, { width: colQty, align: 'center' });
                                    doc.text(`${curr}${Number(it.price || 0).toFixed(2)}`, pageLeft + colDesc + colQty, y + 6, { width: colPrice, align: 'right' });
                                    doc.text(`${curr}${total.toFixed(2)}`, pageLeft + colDesc + colQty + colPrice, y + 6, { width: colTotal - 10, align: 'right' });
                                    y += 22;
                                  ...
                                  y += 20;
                                  doc.rect(pageLeft, y, pageWidth, 60).fill('#f8fafc');
                                  doc.rect(pageLeft, y, 5, 60).fill(theme.primary);
                                  doc.fillColor(theme.primary).fontSize(30).text(`${curr}${Number(amountPaid).toFixed(2)}`, pageLeft + 20, y + 12);
                                  doc.fontSize(10).fillColor('#64748b').text('Total Payment Confirmed', pageLeft + pageWidth - 170, y + 25);
