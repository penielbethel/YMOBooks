...
function drawInvoiceByTemplate(doc, company, invNo, invoiceDate, dueDate, customer, items) {
  ...
  if (template === 'classic') {
    ...
    doc.fontSize(30).fillColor('#000').text((company.name || company.companyName || 'Company'), pageLeft, doc.page.margins.top);
    doc.fontSize(70).fillColor(theme.primary).text('INVOICE', pageLeft, doc.page.margins.top, { align: 'right', width: pageWidth });
    // Accent bar
    doc.save();
    ...
    doc.fontSize(70).fillColor(theme.primary).text('BILL TO', pageLeft, yTop);
    doc.fontSize(30).fillColor('#333');
    ...
    doc.moveDown(0.5);
    doc.fontSize(70).fillColor(theme.primary).text('Invoice', pageLeft, doc.y);
    doc.fontSize(30).fillColor('#333');
    ...
    if (company.bankName || company.accountName || company.accountNumber) {
      doc.moveDown(0.2);
    doc.fontSize(70).fillColor('#000').text('Bank Details', { width: colWidth - 20 });
    doc.fontSize(30).fillColor('#333');
    ...
    doc.restore();
    doc.fillColor('#ffffff').fontSize(70).text((company.name || 'Company'), doc.page.margins.left + 6, doc.page.margins.top + 2, { continued: true });
    if (company.companyId) {
      doc.fillColor('#ffffff').text(` • ${company.companyId}`);
    }
    doc.fillColor(theme.accent).fontSize(70).text('Invoice', { align: 'right' });
    doc.fontSize(30).fillColor('#000').text(`Invoice No: ${invNo}`, { align: 'right' });
    ...
    doc.moveDown(1);
    doc.fontSize(70).fillColor(theme.primary).text('Bill To:', { underline: template === 'classic' });
    doc.fontSize(30).fillColor('#333');
  ...
  // Highlight contact block for quick scanning
    doc.fontSize(70).fillColor(theme.primary).text('WRITE US');
    doc.fillColor('#333').fontSize(30);
  if (company.address) doc.text(company.address);
  if (company.email) doc.text(company.email);
  if (company.phone) doc.text(company.phone);
  ...
  doc.restore();
    doc.fillColor('#000').fontSize(70).text('Description', startX + 8, tableTop + 6, { width: colDesc });
  ...
  items.forEach((it, idx) => {
    ...
    doc.fillColor('#333').fontSize(30).text(String(it.description || ''), startX + 8, y + 6, { width: colDesc });
  ...
  doc.moveTo(startX, y + 8).lineTo(doc.page.width - doc.page.margins.right, y + 8).stroke(theme.accent);
    doc.fontSize(70).fillColor(theme.primary);
  ...
  doc.moveDown(0.5);
    doc.fontSize(30).fillColor('#333').text(`Amount in words: ${amountInWordsWithCurrency(grandTotal, curr)}`);

  // Footer
  // Electronic generation notice with printed date
  doc.moveDown(0.5);
    doc.fontSize(30).fillColor('#555').text(
  ...
  try {
    ...
    if (sigBuf) {
      doc.moveDown(1);
    doc.fontSize(30).fillColor('#333').text('Authorized Signature');
  ...
  if (company.termsAndConditions && String(company.termsAndConditions).trim()) {
    doc.moveDown(1);
    doc.fontSize(70).fillColor(theme.primary).text('Terms and Conditions');
    doc.moveDown(0.2);
    doc.fontSize(30).fillColor('#333').text(String(company.termsAndConditions), {
  ...
  doc.moveDown(1);
    doc.fontSize(30).fillColor('#777').text('Powered by YMOBooks', { align: 'right' });
}
...
function drawReceiptByTemplate(doc, company, rctNo, receiptDate, invoiceNumber, customer, amountPaid) {
  ...
  doc.restore();
    ...
    doc.fillColor('#ffffff').fontSize(30).text((company.name || 'Company'), doc.page.margins.left + 6, doc.page.margins.top + 2, { continued: true });
  if (company.companyId) {
    doc.fillColor('#ffffff').text(` • ${company.companyId}`);
  }

  // Right-side Receipt meta
    doc.fillColor(theme.accent).fontSize(70).text('Receipt', { align: 'right' });
    doc.fontSize(30).fillColor('#000').text(`Receipt No: ${rctNo}`, { align: 'right' });
  ...
  // Customer section
  doc.moveDown(1);
    doc.fontSize(70).fillColor(theme.primary).text('Received From:', { underline: template === 'classic' });
    doc.fontSize(30).fillColor('#333');
  if (customer?.name) doc.text(customer.name);
  if (customer?.address) doc.text(customer.address);
  if (customer?.contact) doc.text(`Contact: ${customer.contact}`);
  ...
  const desc = invoiceNumber ? `Payment for ${invoiceNumber}` : 'Payment received';
    doc.fillColor('#333').fontSize(30).text(desc, startX + 8, y + 6, { width: colDesc });
  doc.text(`${curr}${Number(amountPaid || 0).toFixed(2)}`, startX + 8 + colDesc, y + 6, { width: colAmt, align: 'right' });
  y += rowHeight;

  // Amount in words
  doc.moveDown(0.5);
    doc.fontSize(30).fillColor('#333').text(`Amount in words: ${numberToWords(Number(amountPaid || 0))}`);
