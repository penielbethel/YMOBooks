          ...
          if (res.pdfUrl) {
            try {
              if (Platform.OS === 'web') {
                ...
                if (!cacheDir.endsWith('/')) cacheDir += '/';
                const downloadUri = `${cacheDir}${safeName}`;
                const dl = await FileSystemLegacy.downloadAsync(res.pdfUrl, downloadUri);
                targetUri = dl.uri;
                setPdfReadyUri(targetUri);
              }
            } catch (dlErr) {
              console.warn('[PDF Sync] Backend download failed, falling back to local print:', dlErr);
              // targetUri remains the local one generated by Print.printToFileAsync if it exists
            }
          } else if (Platform.OS === 'web' && file?.uri) {
             // Fallback for web if no server url
